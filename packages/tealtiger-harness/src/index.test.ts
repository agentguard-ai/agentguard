...
import { apply, TealTigerHarnessService, type TealTigerReceipt } from './index';

type TestGuard = (execution: Readonly<ToolExecution>) => string | undefined;
...
describe('TealTigerHarnessService', () => {
  ...
  it('accumulates costs and denies calls over the session budget', async () => {
    ...
    expect(tools.guardCallback?.(secondExecution)).toBe(secondReceipt.reason);
  ...
  it('emits a sanitized receipt before denying execution', async () => {
    ...
    const decision = await ctx.waterfall('tools/pre-execute', execution, () =>
  ...
  it('denies frozen tools in every governance mode', async () => {
    for (const mode of ['ENFORCE', 'MONITOR', 'REPORT_ONLY'] as const) {
      const ctx = new Context();
      const tools = new TestTools(ctx);
