...
// Local patterns intentionally keep pre-execution secret checks offline and deterministic.
...
function resolveConfig(input: Config): ResolvedConfig {
  ...
  if (
    config.sessionBudgetUsd !== undefined &&
    config.defaultToolCostUsd === undefined
  ) {
    throw new Error(
  ...
  return Object.freeze({
    ...
    sessionBudgetMicroUsd:
      config.sessionBudgetUsd !== undefined
        ? toMicroUsd(config.sessionBudgetUsd)
        : undefined,
    defaultToolCostMicroUsd:
      config.defaultToolCostUsd !== undefined
        ? toMicroUsd(config.defaultToolCostUsd)
        : undefined,
...
export class TealTigerHarnessService extends Service {
  ...
  private readonly deniedExecutions = new WeakMap<
  ...
  constructor(ctx: Context, input: Config = {}) {
    ...
    this.engine = new TealEngine(createToolPolicy(this.config), {
      mode: {
  ...
  public isAllowed(toolName: string): boolean {
    ...
    return (
      this.config.allowedTools.has('*') ||
      this.config.allowedTools.has(toolName)
    );
  }

  private toolCostMicroUsd(toolName: string): number | undefined {
    return (
      this.config.toolCostsMicroUsd.get(toolName) ??
      this.config.defaultToolCostMicroUsd
    );
  }
  ...
  public async evaluateTool(
    ...
    const allowlistViolation =
      this.mode !== 'REPORT_ONLY' && !frozen && !this.isAllowed(execution.name);
    ...
    if (!frozen && this.mode !== 'REPORT_ONLY') {
      ...
      if (argumentsText === undefined) {
        ...
        if (this.config.piiDetection) {
          const piiResult = await this.piiGuardrail.evaluate(argumentsText);
    ...
    const receipt: TealTigerReceipt = Object.freeze({
      ...
      cost: Object.freeze({
        ...
        ...(estimatedCallCostMicroUsd === undefined
          ? {}
          : {
              estimated_call_usd: fromMicroUsd(estimatedCallCostMicroUsd),
            }),
        ...(this.config.sessionBudgetMicroUsd === undefined
          ? {}
          : {
